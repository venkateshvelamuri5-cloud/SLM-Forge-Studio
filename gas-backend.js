...
var SHEETS = {
  ...
  'User_SLM_Assignments': [
    'user_email','user_name','profile_id','slm_name','slm_category',
...
function doGet(e) {
  ...
  try {
    switch (p.action) {
      ...
      case 'getAssignments': return ok(handleGetAssignments(p));
...
function handleDeassignProfile(body) {
  ...
  // Also remove from User_SLM_Assignments sheet
  try {
    var assignSheet = ss.getSheetByName('User_SLM_Assignments');
    if (assignSheet) {
      var aRows = assignSheet.getDataRange().getValues();
      for (var j=aRows.length-1; j>=1; j--) {
        if ((aRows[j][0]||'').toLowerCase().trim()===email && aRows[j][2]===profileId) {
          assignSheet.deleteRow(j+1);
        }
      }
    }
  } catch(e) { Logger.log('User_SLM_Assignments deassign error: '+e); }
...
function handleAssignProfile(body) {
  ...
  // Write the new assignments to Access_Control
  var added = 0;
  ...
  // Also maintain User_SLM_Assignments sheet for admin visibility
  try {
    var assignSheet = ensureSheet(ss, 'User_SLM_Assignments');
    var usersSheet  = ss.getSheetByName('Users');
    ...
    if (usersSheet) {
      var uRows = usersSheet.getDataRange().getValues();
      for (var i=1;i<uRows.length;i++) {
        userNames[(uRows[i][0]||'').toLowerCase().trim()] = uRows[i][1]||uRows[i][0]||'';
      }
    }
    var slmName='', slmCat='';
    if (profSheet) {
      ...
      for (var i=1;i<pRows.length;i++) {
        if (pRows[i][0] === pid) { slmName=pRows[i][1]||''; slmCat=pRows[i][3]||''; break; }
      }
    }

    // Delete existing rows for this profile then re-insert
    var existingAssign = assignSheet.getDataRange().getValues();
    for (var i=existingAssign.length-1; i>=1; i--) {
      if (existingAssign[i][2] === pid) assignSheet.deleteRow(i+1);
    }
    emails.filter(Boolean).forEach(function(e) {
      var em = e.toLowerCase().trim();
      assignSheet.appendRow([em, userNames[em]||em, pid, slmName, slmCat, now, by, 'active']);
    });
  } catch(e) { /* non-critical — log but don't fail */ Logger.log('User_SLM_Assignments update error: '+e); }
...
//  USERS LIST
// ═══════════════════════════════════════════════════════════
function handleGetAssignments(p) {
  // Returns all assignments from User_SLM_Assignments sheet
  // Optional filter: ?email=xxx to get one user's assignments
  var ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  var sheet = ss.getSheetByName('User_SLM_Assignments');
  if (!sheet) return { success:true, assignments:[] };
  var rows = sheet.getDataRange().getValues();
  var filterEmail = p.email ? p.email.toLowerCase().trim() : null;
  var assignments = [];
  for (var i=1; i<rows.length; i++) {
    var em = (rows[i][0]||'').toLowerCase().trim();
    if (filterEmail && em !== filterEmail) continue;
    assignments.push({
      userEmail:  em,
      userName:   rows[i][1]||em,
      profileId:  rows[i][2]||'',
      slmName:    rows[i][3]||'',
      slmCategory:rows[i][4]||'',
      assignedAt: rows[i][5]||'',
      assignedBy: rows[i][6]||'',
      status:     rows[i][7]||'active',
    });
  }
